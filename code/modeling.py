...
from generation import SelfDebiasingLlamaLMHeadModel, SelfDebiasingGPT2LMHeadModel
...
class LlamaWrapper(ModelWrapper):
    ...
    def __init__(self, model_name: str = "/home/doubleyyh/models/vicuna-13b-1.1", use_cuda: bool = True):
        """
        :param model_name: the name of the pretrained vicuna-13b-1.1 model (default: "vicuna-13b-1.1")
        :param use_cuda: whether to use CUDA
        """
        super().__init__(use_cuda=use_cuda)
        self._tokenizer = LlamaTokenizer.from_pretrained(model_name)
        self._model = SelfDebiasingLlamaLMHeadModel.from_pretrained(model_name, device_map="auto", load_in_4bit=True)
        ##self._model = SelfDebiasingGPT2LMHeadModel.from_pretrained(model_name)  # type: SelfDebiasingGPT2LMHeadModel
        self._tokenizer.pad_token = self._tokenizer.eos_token
        self._model.config.pad_token_id = self._tokenizer.eos_token_id
